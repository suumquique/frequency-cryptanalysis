// Отключаем ооповещение об ошибке при пропуске проверки значения, возвращаемого функцией scanf
...
#define STEPS 35
// Максимальная длина слова в текста
#define MAX_WORD_LENGTH 30
// Максимальное количество слов в текста
#define MAX_WORD_COUNT 200
// Максимальная длина текста в символах
#define MAX_FILE_LENGTH 10000
// Разделители, по которым предложение разбивается на отдельные слова
#define delimeters " ,-.:;\n"
...
// Буквы русского алфавита, расположенные в частотном порядке (буквы, встречающиеся в русском языке чаще, идут сначала)
const char* frequency = "оеаинтсрвлкмдпуяыьгзбчйхжшюцщэфёъ";
// История измнений, в каждом предыдущем элементе массива хранится полный текст до текущей замены
char history[STEPS][MAX_FILE_LENGTH];
// Текущий "шаг" истории, то есть индекс актуального текста в массиве history
...
void main(void) {
	...
	// Считываем входной файл посимвольно и записываем текст в начальный (нулевой) элемента массива с историей
	...
	// Записываем посимвольно итоговый текст (с текущего индекса массива со всеми снапшотами текстов) в выходной файл
...
bool isLetter(char symbol) {
	// Поскольку Ё и ё имеют ascii-коды, не находящиеся внутри диапазона обычного русского алфавита, их требуется проверять отдельно
...
// Сортируем слова из зашифрованного текста по их длине и выводим блоками (для каждой длины - 1 блок)
void groupByWordLength() {
	size_t wordCount, i;
...
/* Функция считает количество зашифрованных букв в слове. 
Зашифрованными буквами в нашем случае считаются буквы русского алфавита в верхнем регистре (прописные буквы) */
...
// Сортирует слова из текущего зашифрованного текста по количеству зашифрованных букв в них и выводит блоками (сначала те, где зашифрованных больше)
void groupByEncryptedLettersNumber() {
	size_t wordCount, i;
	char words[MAX_WORD_COUNT][MAX_WORD_LENGTH];
	char currentString[MAX_FILE_LENGTH];
...
/* Двигаемся по истории зашифрованных текстов назад на определенное количество шагов.
* Функция применяется в том случае, если пользователь совершил ошибочную замену и ему требуется откатить изменения.
* Если указано число шагов, превышающее количество сохраненных снапшотов, идёт откат к изначальному состоянию полностью зашифрованного текста. */
void goBack(size_t steps) {
	if (!steps) {
...
void replaceLetters(char* replaceString) {
	...
	if (replaceString == NULL) {
		...
		gets_s(replaceString, alphabetCardinality * 5 - 1);
	}
...
/* Добавляет символ (букву) в счетчик букв. Если буква уже есть в массиве, то счетчик увеличивается, 
если еще нет - добавляется, а счетчику присваивается значение "1" */
void addOrUpdate(CounterPtr counter, char letter) {
	while (counter->count) {
...
/*Выводит столбиком самые часто встречающиеся зашифрованные буквы в тексте и сопоставляет каждой букву русского алфавита,
стоящую на том же месте по частоте в массиве frequency.*/
void analyseText() {
...
/* Автоматически заменяет все зашифрованные буквы в тексте на аналогичные по частоте из обычного алфавита.
* Например, самую часто встречающуюся букву в зашифрованном тексте заменит на русскую "о" (frequency[0]), 
* так как "о" чаще всего встречается в русском языке */
void replaceByFrequencyAlgorithm() {
	Counter counter[alphabetCardinality * 2];
	for (size_t i = 0; i < alphabetCardinality * 2; i++) counter[i].count = 0;
	for (size_t i = 0; history[currentStep][i]; i++) if (isLetter(history[currentStep][i])) addOrUpdate(&counter, history[currentStep][i]);
